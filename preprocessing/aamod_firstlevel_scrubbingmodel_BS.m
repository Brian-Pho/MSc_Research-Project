function [aap,resp]=aamod_firstlevel_scrubbingmodel_BS(aap, task, subjInd)
%
    ...
    case 'doit'
        ...
        for thisSess = 1:numSess     
            ...
            if (settings.includeCSF || settings.includeWM || settings.includeglobal) && aas_stream_has_contents(aap, 'compSignal')
                ...
                if settings.includeglobal %includeGlobal - This was orginal
            ...
            if includeMovement && aas_stream_has_contents(aap, 'realignment_parameter')
                ...
                if volterraMovement
                    ...
                    %  M = spm_Volterra(U, [1 0 0; 1 -1 0; 0 1 -1]', 2);     Original
                    M = spm_Volterra(U, [1 0 0; 1 -1 0]', 2); %This removes time lag (likely spin history effects) Not used by http://onlinelibrary.wiley.com/doi/10.1002/hbm.23653/full
                end
...
%                         Sv = full(Sv);
%                     end
%                     Sv = diag(Sv).^2; S = cumsum(Sv)/sum(Sv);
%                     Npv = find(Sv > .99); Npv = Npv(1);
%                     aas_log(aap, 0, sprintf('%d modes from %d confounds (%.2f percent variance of correlation explained)', Npv, size(M,2), 100*S(Npv)))
%                     X0v = full(Uv(:,1:Npv));
%                     Ncv = Npv;
%                     
...
%                     M = X0v;
%                     Cnamesv = {};
%                     for svdRegv = 1:size(X0v,2)
%                         Cnamesv{end+1} = sprintf('confound SVD %d', svdRegv);
%                     end
%                 end
        ...
        aap = aas_desc_outputs(aap,subjInd,subjSessionI,'epi',allfiles);
